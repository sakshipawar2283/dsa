class AES:
    ...
    def getsbox(self,sboxlist):
        ...
        for s in sboxlist:
            ...
            substituted_sbox.append(temp)
        return substituted_sbox
    
def generate_key(self,hex_key,round_no):
    if (round_no<10):
        w0=[]
        w1=[]
        w2=[]
        ...
        for i in range(4):
            w0.append(hex_key[i])
            w1.append(hex_key[i+4])
            w2.append(hex_key[i+8])
            w3.append(hex_key[i+12])
        ...
        substitutsbox=self.getsbox(w3)
        w3=list(substitutsbox)

        xored=[]

        for i in range(4):
            round_no=self.round_constant[round_no]
            a=w3[i]
            sum=int(a,16)^round_no[i]
            xored.append(hex(sum).replace("ox"," "))
        w3=list[xored]
        def rounkeyxor(w00,w33):
            w=[]*4
            for i in range(4):
